...
import { saveMetaData, saveChunk, getMetaData, getFilesInRoom, getLastChunkIndex, deleteFile, updateFileProgress, streamFileToDownload, type FileMetadata } from "./ProgressDB";
...
function App() {
        ...
        case "ack":
          if (sendingFile.current) {
            ...
            if (currentChunkToSend.current < totalChunks) {
              ...
              setSendQueue((prev) =>
                prev.map((f) =>
                  f.id === sendingFile.current?.id ? { ...f, status: "sent", progress: 100 } : f
                )
              );
  ...
  const handleBinaryChunk = async (data: ArrayBuffer) => { 
    ...
    const progress = Math.round(((nextIndex + 1) / currentMeta.current.totalChunks) * 100);
  ...
  const processNextFileInQueue = () => {
    ...
    setSendQueue((prev) =>
      prev.map((f) =>
        f.id === nextFile.id ? { ...f, status: "sending" } : f
      )
    );
